defmodule SolarisCore.Infrastructure.Schemas.PlannedTransactionSchema do
  ...
  def changeset(planned_transaction, attrs) do
    planned_transaction
    |> cast(attrs, [:description, :amount, :type, :category_id, :payment_method, :day_of_month, :starts_on, :active, :notes])
    |> validate_required([:description, :amount, :type, :category_id, :payment_method, :day_of_month, :starts_on, :active])
