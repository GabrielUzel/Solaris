defmodule SolarisCore.Application.Commands.RemoveManualTransactionFromBudgetMonth do
  ...
  def execute(budget_month_id, transaction_id) do
    with {:ok, budget_month} <- BudgetMonthRepo.get(budget_month_id),
         {:ok, updated} <- BudgetMonth.remove_manual_transaction(budget_month, transaction_id) do
