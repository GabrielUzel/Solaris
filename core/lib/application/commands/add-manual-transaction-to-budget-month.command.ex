defmodule SolarisCore.Application.Commands.AddManualTransactionToBudgetMonth do
  ...
  def execute(budget_month_id, attrs) do
    with {:ok, budget_month} <- BudgetMonthRepo.get(budget_month_id),
         ...
         transaction_attrs <- build_transaction_attrs(attrs, category),
  ...
  defp build_transaction_attrs(attrs, category) do
    attrs
